...
pub fn ckd_private_parent_to_private_child_key(
  ...
  index: u32,
  depth: u8
) -> ChildPrivateKeyChainCodeAndzprv {
  let base: u32 = 2;
  ...
  sk.add_assign(&parent_private_key).unwrap();
  ...
  ChildPrivateKeyChainCodeAndzprv {
    child_chain_code: hex::decode(child_chain_code).unwrap(),
    child_private_key: hex::decode(&child_private_key).unwrap(),
 ...
 /// Child Key Derivation (CKD): Parent Public Key to Child Public Key.
  ...
  pub fn ckd_public_parent_to_public_child_key(
    ...
    index: u32,
    depth: u8
  ) -> ChildPublicKeyChainCodeAndzpub {
    let base: u32 = 2;
...
/// assert_eq!(index, 2_147_483_732u32); // 2^31 + 84
/// ```
///  
pub fn get_normal_or_hardened_index(index: &str) -> u32 {
  if index.contains("'") {
...
/// ```
///
fn get_fingerprint(public_key: String) -> Vec<u8> {
